<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Facilities | K.D Hostel Management System</title>

    <!-- Bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.7/dist/css/bootstrap.min.css" rel="stylesheet">

    <!-- Font Awesome -->
    <link rel="stylesheet"
          href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css">

    <!-- Modern CSS -->
    <link rel="stylesheet" href="css/modern.css">

</head>

<body>

<!-- ================= MODERN NAVBAR ================= -->

<nav class="navbar navbar-expand-lg navbar-modern" id="mainNavbar">
    <div class="container">
        <!-- Logo -->
        <a class="navbar-brand navbar-brand-modern" href="index.php">
            <div class="logo-icon">
                <i class="fa-solid fa-building"></i>
            </div>
            <div class="brand-text">
                <span class="brand-primary">K.D Polytechnic Hostel</span>
                <span class="brand-secondary">Patan</span>
            </div>
        </a>

        <!-- Mobile Menu Button -->
        <button class="navbar-toggler" type="button"
                data-bs-toggle="collapse"
                data-bs-target="#navbarMenu"
                aria-controls="navbarMenu"
                aria-expanded="false"
                aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <!-- Menu -->
        <div class="collapse navbar-collapse" id="navbarMenu">
            <ul class="navbar-nav ms-auto align-items-center gap-2">
                <li class="nav-item">
                    <a class="nav-link nav-link-modern" href="index.php">Home</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-modern" href="about.php">About</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-modern active" href="facilities.php">Facilities</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-modern" href="gallery.php">Gallery</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link nav-link-modern" href="contact.php">Contact</a>
                </li>
                <li class="nav-item ms-lg-2">
                    <a href="login.php" class="btn btn-login-cta">
                        <i class="fa-solid fa-right-to-bracket me-2"></i>Login
                    </a>
                </li>
            </ul>
        </div>
    </div>
</nav>

<!-- ================= MODERN PAGE HEADER ================= -->

<section class="hero-modern" style="min-height: 40vh;">
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <div class="hero-badge animate-fade-in-up">
                    <i class="fa-solid fa-star"></i>
                    Our Amenities
                </div>
                <h1 class="hero-title animate-fade-in-up animate-delay-1" style="font-size: 2.5rem;">
                    Hostel Facilities
                </h1>
                <p class="hero-subtitle animate-fade-in-up animate-delay-2" style="margin: 0 auto;">
                    Modern Facilities for Better Student Life
                </p>
            </div>
        </div>
    </div>
</section>

<!-- ================= FACILITIES ================= -->

<section class="container py-5">

<div class="row g-4">

<div class="col-md-4 animate-fade-in-up">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-wifi"></i>
    </div>
    <h4 class="card-title">Free WiFi</h4>
    <p class="card-text">High-speed internet available 24×7 throughout the hostel.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-1">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-book"></i>
    </div>
    <h4 class="card-title">Library</h4>
    <p class="card-text">Peaceful study environment with books and reference materials.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-2">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-utensils"></i>
    </div>
    <h4 class="card-title">Mess</h4>
    <p class="card-text">Fresh, hygienic and nutritious breakfast, lunch and dinner.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-shirt"></i>
    </div>
    <h4 class="card-title">Laundry</h4>
    <p class="card-text">Regular laundry service for hostel students.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-1">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-square-parking"></i>
    </div>
    <h4 class="card-title">Parking</h4>
    <p class="card-text">Secure parking area for two-wheelers and bicycles.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-2">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-shield-halved"></i>
    </div>
    <h4 class="card-title">24×7 Security</h4>
    <p class="card-text">CCTV surveillance and security guards available all day.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-dumbbell"></i>
    </div>
    <h4 class="card-title">Gym</h4>
    <p class="card-text">Modern fitness equipment for students' health and wellness.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-1">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-notes-medical"></i>
    </div>
    <h4 class="card-title">Medical Facility</h4>
    <p class="card-text">Basic first aid and emergency medical assistance available.</p>
</div>

</div>

<div class="col-md-4 animate-fade-in-up animate-delay-2">

<div class="card-modern text-center h-100">
    <div class="card-icon">
        <i class="fa-solid fa-book-open-reader"></i>
    </div>
    <h4 class="card-title">Study Room</h4>
    <p class="card-text">Quiet study rooms for focused learning and group discussions.</p>
</div>

</div>

</div>

</section>

<!-- ================= MODERN FOOTER ================= -->

<footer class="footer-modern">
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <p class="mb-0 text-white-50">© 2026 K.D Hostel Management System | All Rights Reserved</p>
            </div>
        </div>
    </div>
</footer>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.7/dist/js/bootstrap.bundle.min.js"></script>

<!-- Navbar Scroll Effect -->
<script>
    window.addEventListener('scroll', function() {
        const navbar = document.getElementById('mainNavbar');
        if (window.scrollY > 50) {
            navbar.classList.add('scrolled');
        } else {
            navbar.classList.remove('scrolled');
        }
    });
</script>

</body>

</html>